...
class TestChessTimer(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for i in [i for i in current_list_module if i not in self._list_module]:
            del sys.modules[i]
        if 'chess_timer' in sys.modules.keys():
    ...
    def test_init(self):
        ...
        init_state = {player:timedelta(microseconds=0).microseconds for player in ChessTimer._players}
        ...
        with patch('datetime.datetime') as mock_datetime:
            ...
            state = ChessTimer(
                dict(
                    ...
                    start_current_step_time = current_time.isoformat()
                )
            ).step()
            self.assertEqual(state['current_player'], 0)
    ...
    def test_step(self):
        ...
        with patch('datetime.datetime') as mock_datetime:
            ...
            from chess_timer import ChessTimer
            state = ChessTimer(dict(
                current_player=0,
                start_current_step_time=(old_time).isoformat()
                )
            ).step()
            self.assertEqual(state['current_player'], 1)
            self.assertEqual(state[ChessTimer._players[0]], (current_time-_get_time(old_time.isoformat())).total_seconds())

    def test_get_statistic(self):
        from datetime import timedelta
        from chess_timer import ChessTimer
        state = ChessTimer(dict(Red=timedelta(minutes=10).total_seconds())).get_statistic()
