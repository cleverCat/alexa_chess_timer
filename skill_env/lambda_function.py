...
JOKE_LIST = [
    'May the Force be with {0}', 'Stay awhile and listen', 'Hold his beer',
    'As the prophecy foretold', 'It is super effective', 'Party is over',
    'Brace yourselves, {0} is coming',
    'I could calculate your chance of survival, {0}, but you will not like it”',
    'Release the Kraken', 'Just as planned',
    'Oh what a day. What a lovely day', 'Look at {0}. He is the captain now',
    'Are you not entertained?', 'May the odds ever be in your favor',
    'Why so serious, {0}?', 'Just keep swimming, {0}', 'You are a wizard, {0}',
    'Here is {0}!', 'A wild {0} appeared',
    'The only way to win is not to play, {0}', 'Do you feel in charge, {0}?'
]
...
def get_statistic(session):
    ...
    def get_statistic_time(_time):
        ...
        return 'minutes ' + str(current_minutes) + ' seconds ' + str(
            current_seconds)

    statistic = chess_timer.get_statistic()
    list_statistic = [
        str(player) + ' ' + get_statistic_time(_time) + ' '
        for player, _time in statistic.items()
    ]
...
class GetStepHandle(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        handler_input.response_builder.speak(
            (STEP_MESSAGE + ' ' + random.choice(JOKE_LIST)).format(
                new_session['current_player_name'])).ask('hello')
        return handler_input.response_builder.response
