...
class ChessTimer(object):
    ...
    def __init__(self, state: dict):
        ...
        self._state = {
            key: state[key] if key in state else init_state[key]
    ...
    def step(self):
        ...
        if self._state['current_player'] is None:
            self._state['current_player'] = 0
            self._state['start_current_step_time'] = current_time
            self._state['current_player_name'] = self._players[
        ...
        time_delta = current_time - _get_time(
            self._state['start_current_step_time'])
        self._state[
            self._players[current_player]] += time_delta.total_seconds()
    ...
    def get_statistic(self):
        '''получаем статистику сколько времени ходил каждый игрок
        для игрока ход которого все еще идет получаем статистику
        ...
        def get_statistic_for_player(player):
            ...
            current_time = datetime.now()
            current_time_delta = (current_time - _get_time(
                self._state['start_current_step_time'])).total_seconds()
            return self._state[player] + current_time_delta

        return {
            player: get_statistic_for_player(player)
